...
function ListItem(props) {
  const removeList = (item) => {
    axios.delete("http://localhost:3001/items2/" + item.id).then(() => {
  ...
  return (
    <ul className="todo__items">
      {props.items.map((item) => (
        <li
          key={item.id}
          onClick={props.toggle ? props.toggle : () => props.onClickItem(item)}
          className={classNames({
            active: props.item && item.id === props.item.id,
          })}
        >
          {/* <Link to={item.id === 1111 ? `/` : `/lists/${item.id}`}> */}
          {item.icon ? (
          ...
          {item.iconX && (
            <i className="iconX" onClick={() => removeList(item)}>
              <svg

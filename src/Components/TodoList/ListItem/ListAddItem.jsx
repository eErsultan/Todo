...
function ListAddItem({ onAdd, color }) {
  ...
  const addList = () => {
    setLoading(true);
    axios
      .post("http://localhost:3001/items2", {
        title: inputValue,
        color: colIcon,
        iconX: true,
        complete: false,
  ...
  return (
    <div className="item-add">
      <ListItem items={itemAdd} toggle={() => setState(true)} />
      {state && (
        <div className="popup-add-list">
          <i className="popup-exit" onClick={closePopup}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="21"
              height="21"
          ...
          <div className="color-icon">
            {color.map((item, index) => (
              <ColorIcon
                item={item}
                key={index}
                onClick={() => setColIcon(item.color)}
                className={classNames({ active: colIcon === item.color })}
              />
            ))}
          </div>
